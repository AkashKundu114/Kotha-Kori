...
(`migrations/0001_init.sql`, plus `0004_seller_profile.sql` for the new
Pricing agent's table) — no separate migration step needed for a fresh deploy.
...
Everything else in `.env.example` (S3 bucket, Langfuse, mandi price API,
Flux Pro, Bengali font for poster generation) is optional.
...
Bengali message instead of a crash.

Full design rationale: see `docs/architecture.md` (especially §8 for the
OpenAI removal) and `docs/security.md` for what's hardened and why.
...
tests/unit/            Fast, offline tests for the security-critical and pricing logic
```
