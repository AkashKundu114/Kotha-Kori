...
async def _apply_correction(state: ConversationState, pending: dict, correction_text: str, turns: int) -> dict:
    ...
    return {
        ...
        "awaiting_confirmation": True,
        "ledger_confirmation_turns": turns,
        "outbound_messages": [{"type": "text", "body": f"ঠিক আছে, আবার দেখুন:\n\n{_build_confirmation(updated)}"}],
    ...
    if user_id:
        async with get_db_session() as db:
            for tx in pending.get("transactions", []):
                amt = float(tx.get("amount_inr", 0) or 0)
                entry = LedgerEntry(
                    user_id=user_id,
                    ...
                    unit=tx.get("unit"),
                    raw_transcript=pending.get("raw_transcript"),
                    is_corrected=pending.get("extracted_by") is not None and state.get("ledger_confirmation_turns", 0) > 0,
