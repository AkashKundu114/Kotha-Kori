...
from services.vision_service.vision_router import (
...
async def catalog_node(state: ConversationState) -> dict:
    ...
    try:
        ...
        return {
            "outbound_messages": [
                {"type": "text", "body": "ছবিটা লোড করতে সমস্যা হয়েছে। আবার পাঠান।"}
            ],
    ...
    if len(raw_bytes) > MAX_IMAGE_BYTES:
        return {
            "outbound_messages": [
                {"type": "text", "body": "ছবিটা অনেক বড়। একটু ছোট সাইজে পাঠান।"}
            ],
            "trace": ["catalog_node:oversized_image"],
        }

    processed_bytes, quality_error = await asyncio.to_thread(
        process_product_image, raw_bytes
    )
    if quality_error:
        return {
    ...
    caption, (price_min, price_max) = await generate_caption(
        product_info, shg_name=_shg_name(state)
    )

    processed_key = (
        f"catalog/{state.get('user_id', 'unknown')}/{uuid.uuid4().hex[:10]}.png"
    )
    ...
    processed_url = s3.generate_presigned_url(
        "get_object",
        Params={"Bucket": s.s3_bucket, "Key": processed_key},
        ExpiresIn=86400,
    )

    await _record_creation(
        state, raw_key, processed_key, product_info, caption, price_min, price_max
    )
    ...
    return {
        ...
        "outbound_messages": [
            ...
            {
                "type": "text",
                "body": "ইংরেজিতেও ক্যাপশন চান? (শহুরে কাস্টমারদের জন্য) — 'হ্যাঁ' লিখুন।",
            },
        ],
