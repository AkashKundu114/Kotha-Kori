...
| User model / personalization | Not mentioned anywhere in v1 or v2 | **New requirement** — see `docs/research.md` |
...
| 2 | Idempotency + rate limiting (see ../security/security.md H1/H2) | Paste the H1/H2 remediation spec directly |
...
| Day | Work |
|---|---|
| 6–7 | Implement user model (schema + onboarding Flow + injection into `model_router` prompts) — full spec in `../../research.md` |
| 8 | Apply P0 security fixes from `../security/security.md` (idempotency, rate limit, audio deletion job, input size caps, spam guard) |
