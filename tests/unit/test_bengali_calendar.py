...
def test_new_year_boundary_transitions_correctly():
    ...
    assert day_before[0] == "চৈত্র"       # last month of the old Bangla year
    assert new_year_day[0] == "বৈশাখ"     # first month of the new Bangla year
    assert new_year_day[1] == 1           # first day of the month
    assert new_year_day[2] == day_before[2] + 1  # bangla year increments
...
def test_result_is_always_structurally_valid_across_a_four_year_span_including_a_leap_year():
    ...
    while d <= end:
        ...
        assert 1 <= day <= 31
        assert bangla_year > 1400  # sanity floor, not a precise assertion
        d += timedelta(days=1)


def test_bangla_year_matches_well_known_public_mapping():
    # Widely cited public mapping: Gregorian 2026 (after mid-April) falls in
    # Bangla year 1433. This is a sanity check against common knowledge, not
    # a claim of astronomical precision — see the module's verification note.
...
def test_format_bangla_calendar_label_includes_approximation_marker():
    ...
    assert "আনুমানিক" in label  # "approximate" — must never be silently dropped
    assert any(month in label for month in BANGLA_MONTHS)


def test_leap_year_day_does_not_crash():
    # Feb 29 only exists in a leap year — confirms the conversion handles it.
